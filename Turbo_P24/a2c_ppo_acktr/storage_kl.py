...
class RolloutStorageKL(object):
    def __init__(self, num_env_steps, num_steps, num_processes, obs_shape, action_space, max_new_tokens):
        ...
        self.kl_penalty = torch.zeros(num_steps, num_processes, 1)
    ...
    def to(self, device):
        ...
        self.kl_penalty = self.kl_penalty.to(device)
        self.diversities = self.diversities.to(device)

    def insert(self, obs, output_ids, actions, action_log_probs,
               value_preds, rewards, masks, bad_masks, format_rewards, diversity_score, kl):
        ...
        self.kl_penalty[self.step].copy_(kl)
        ...
        if use_proper_time_limits:
            if use_gae:
                ...
                for step in reversed(range(self.rewards.size(0))):
                    ...
                    self.returns[step] = self.returns[step] + self.format_rewards[step] - self.kl_penalty[step]
            else:
