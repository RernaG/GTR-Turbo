...
class DualSampler(Sampler):
    def __init__(self, ppo_bz, bc_bz, batch_size):
        self.ppo_sampler = SubsetRandomSampler(range(ppo_bz))
        self.bc_sampler = SubsetRandomSampler(range(bc_bz))
        self.batch_size = batch_size
    
    def __iter__(self):
        iter1 = iter(self.ppo_sampler)
        iter2 = iter(self.bc_sampler)
        ...
        for i in range(len(self.ppo_sampler) // self.batch_size):
            ...
            for _ in range(self.batch_size):
                ...
                try:
                    bc_batch.append(next(iter2))
                except StopIteration:
                    iter2 = iter(self.bc_sampler)
                    bc_batch.append(next(iter2))

            yield ppo_batch, bc_batch

    def __len__(self):
        return len(self.ppo_sampler) // self.batch_size


class RolloutStorageSFT(object):
    def __init__(self, num_env_steps, num_steps, num_processes, obs_shape, action_space, max_new_tokens):
        ...
        self.diversities = torch.zeros(num_steps, num_processes, 1)
    ...
    def to(self, device):
        ...
        self.diversities = self.diversities.to(device)
        

    def insert(self, obs, output_ids, actions, action_log_probs,
               value_preds, rewards, masks, bad_masks, format_rewards, input_ids, labels, diversity_score):
        ...
        self.step_accu = self.step_accu + 1
    ...
    def feed_forward_generator(self,
        ...
        bc_batch_size = num_processes * self.step_accu

        sampler = DualSampler(ppo_batch_size, bc_batch_size, mini_batch_size)
        for sample in sampler:
            ppo_indices, bc_indices = sample
            ...
            bc_obs_batch = self.obs_accu[:-1].view(-1, *self.obs_accu.size()[2:])[bc_indices].to(self.input_ids.device)
            input_ids_batch = self.input_ids.view(-1,
                                              self.input_ids.size(-1))[bc_indices]
            labels_batch = self.labels.view(-1, self.labels.size(-1))[bc_indices]
            ...
            yield obs_batch, output_ids_batch, actions_batch, \
                value_preds_batch, return_batch, masks_batch, old_action_log_probs_batch, adv_targ, bc_obs_batch, input_ids_batch, labels_batch
